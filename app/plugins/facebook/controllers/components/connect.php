...
class ConnectComponent extends Object {
	...
	*/
     ...
     * @param array|\for $settings
     * @internal param object $Controller to attach to
     * @internal param \for $settings Connect
     * @return void
    @access public
     */
	...
	*/
		...
		if($Auth->user()){
			...
			elseif(empty($this->authUser) && $this->createUser) {
				...
				if($this->__runCallback('beforeFacebookSave')){
					if (!$this->User->find("count", array("conditions" => array("User.email" => $this->authUser['User']['email'])))) {
                        $this->hasAccount = ($this->User->saveAll($this->authUser, array('validate' => false)));
					} else {
                        $userId = $this->User->find("first",
                            array(
                                "conditions" => array(
                                    "User.email" => $this->authUser['User']['email']
                                )
                            )
                        );
                        $this->User->id = $userId["User"]["id"];

                        $this->hasAccount = ($this->User->saveField('facebook_id', $this->uid));
     ...
     * @param \key|null $field return
     * @internal param \key $field to return (xpath without leading slash)
     * @return null
     */
