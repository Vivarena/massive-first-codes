...
class RatingsController extends RatingAppController {
  ...
  function view($model = '', $id = 0, $options = '') {
    ...
    $this->set('data', array('%VOTES%' => $votes.' '.__n('vote', 'votes', $votes, true),
                             '%RATING%' => $userRating,
                             '%AVG%' => ($optionsData['showVotes'])? $avgRating : $userRating,
                             '%MAX%' => Configure::read('Rating.maxRating')));
    $this->render('view');
  }

  function view_for_form($model = '', $id = 0, $options = '') {
        $this->layout = null;
        ...
        if (empty($optionsData['rating_init'])) {
            $ratingInit = 0;
        }else{
            $ratingInit = $optionsData['rating_init'];
        }
