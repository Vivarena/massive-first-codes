(function($) {
    ...
    function renderSearchResults ($data) {
        ...
        if (num_results) {
            for (var i=0; i<num_results; i++) {
                $("#search_results_table tbody").append("<tr id='row-" + $data[i].id + "'><td>"  + $data[i].first_name + " " +  $data[i].last_name + "</td><td>" + $data[i].email + "</td><td>" + $data[i].pretty_date + "</td><td>" + $data[i].num_views + "</td><td>" + $data[i].num_comments + "</td><td><span  style=\"cursor: pointer;\" onclick=\"showFormDetails(" + $data[i].id + ");\"> [Show]&nbsp;&nbsp;&nbsp; </span><a id='del' href='" + $data[i].id + "'>[Del]</a> </td></tr>");
            }
    ...
    function renderRecentFormSubmissions ($data) {
        ...
        var num_results = $data.length;
        for (var i=0; i<num_results; i++) {
            $("#recent_results tbody").append("<tr id='row-" + $data[i].id + "'><td>"  + $data[i].first_name + " " +  $data[i].last_name + "</td><td>" + $data[i].email + "</td><td>" + $data[i].pretty_date + "</td><td>" + $data[i].num_views + "</td><td>" + $data[i].num_comments + "</td><td><span  style=\"cursor: pointer;\" onclick=\"showFormDetails(" + $data[i].id + ");\"> [Show]&nbsp;&nbsp;&nbsp; </span><a id='del' href='" + $data[i].id + "'>[Del]</a>  </td></tr>");
        }
